(function () {
  ...
  function injectShell() {
    ...
    shell.innerHTML = '<div class="demo-shell-bar"><p>Concepto base · ' + config.packageName + '</p><button type="button" class="demo-scope-toggle" aria-expanded="false" aria-controls="demo-scope-panel">Ver alcance</button></div><section class="demo-scope-panel" id="demo-scope-panel" hidden tabindex="-1"><div><p class="scope-label">Paquete representado</p><h2>' + config.packageName + '</h2><p class="scope-price">' + config.packagePrice + '</p></div><dl><div><dt>Pago</dt><dd>' + config.packageScope.payment + '</dd></div><div><dt>Secciones</dt><dd>' + config.packageScope.sections + '</dd></div><div><dt>Servicios</dt><dd>' + config.packageScope.services + '</dd></div><div><dt>Rondas</dt><dd>' + config.packageScope.revisions + '</dd></div><div><dt>Entrega</dt><dd>' + config.packageScope.delivery + '</dd></div><div><dt>Publicación</dt><dd>' + config.packageScope.publication + '</dd></div><div><dt>Dominio</dt><dd>' + config.packageScope.domain + '</dd></div></dl><div class="scope-columns"><div><h3>Funciones representadas</h3><ul>' + list(config.representedFeatures) + '</ul></div><div><h3>No incluido en este alcance</h3><ul>' + list(config.notIncluded) + '</ul></div></div><p class="scope-notice">' + config.commercialNotice + '</p><div class="scope-actions"><a class="button primary" href="' + nexoWhatsappHref() + '" target="_blank" rel="noopener noreferrer">Quiero una solución similar</a><a class="button ghost" href="' + config.nexoSiteUrl + '" target="_blank" rel="noopener noreferrer">Explorar NEXO26 Digital</a></div></section>';
